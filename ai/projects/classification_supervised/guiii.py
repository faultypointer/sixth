...
# Add category mappings for combobox fields
CATEGORY_MAPPINGS = {
    "Specific Gravity": {b"1.005": 0, b"1.010": 1, b"1.015": 2, b"1.020": 3, b"1.025": 4, b"?": -1},
    "Albumin": {b"0": 0, b"1": 1, b"2": 2, b"3": 3, b"4": 4, b"?": -1},
...
# Update the get_value method for ModernEntry
    def get_value(self):
        """
        Get and validate the value of the input field.
        - For `entry`, ensure the value is not empty and is numeric.
        - For `combobox`, encode the selected value using CATEGORY_MAPPINGS.
        """
        value = self.input.get()
        if isinstance(self.input, ttk.Entry):
            if not value.strip():
                raise ValueError(f"{self.label.cget('text')} cannot be empty.")
            try:
                if self.label.cget('text') in CATEGORY_MAPPINGS:
                    mapping = CATEGORY_MAPPINGS.get(self.label.cget("text"), {})
                    value = mapping.get(value.encode(), 'a')  # Default to -1 if value not found
                return float(value)
            except ValueError:
                print(value)
                raise ValueError(f"{self.label.cget('text')} must be a number.")
        else:  # Combobox
            if not value:
                raise ValueError(f"{self.label.cget('text')} must be selected.")
            # Encode the combobox value
